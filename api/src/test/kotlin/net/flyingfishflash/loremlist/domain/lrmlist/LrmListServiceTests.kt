...
class LrmListServiceTests : FunSpec({
  ...
  test("patch(): update name") {
    ...
    val lrmList = LrmList(name = "lorem list", description = "describe lorem list")
    every { lrmListRepository.findByIdOrNull(id) } returns lrmList
    every { lrmListRepository.save(ofType(LrmList::class)) } just Awaits
  ...
  test("delete(): when list found then lrmListRepository.delete()") {
    val id = 1L
    every { lrmListRepository.findByIdOrNull(id) } returns LrmList(name = "Lorem Ipsum")
    every { lrmListRepository.deleteById(ofType(Long::class)) } returns Unit
    lrmListService.delete(id)
  ...
  test("findByIdOrListNotFoundException(): when list found then list returned") {
    val id = 1L
    val lrmList = LrmList(name = "Lorem Ipsum")
    every { lrmListRepository.findByIdOrNull(id) } returns lrmList
