...
class LrmItemServiceTests : DescribeSpec({
  ...
  val lrmItemService = LrmItemServiceDefault(mockAssociationService, mockLrmItemRepository)
  ...
  describe("deleteAllByOwner()") {
    it("all items deleted") {
      ...
      every { mockAssociationService.deleteByItemOwnerAndItemId(itemId = ofType(UUID::class), itemOwner = ofType(String::class)) } returns mockAssociationServiceResponse
      ...
      verify { mockAssociationService.deleteByItemOwnerAndItemId(itemId = ofType(UUID::class), itemOwner = ofType(String::class)) }
    ...
    it("no lists deleted (none present)") {
      ...
      verify { mockLrmItemRepository.findByOwner(owner = ofType(String::class)) }
      verify(exactly = 0) { mockAssociationService.deleteByItemOwnerAndItemId(itemId = ofType(UUID::class), itemOwner = ofType(String::class)) }
  ...
  describe("deleteByIdAndOwner()") {
    ...
    describe("associated lists") {
      it("item is deleted (removeListAssociations = true)") {
        ...
        every { mockAssociationService.deleteByItemOwnerAndItemId(itemId = id1, itemOwner = ofType(String::class)) } returns mockAssociationServiceResponse
