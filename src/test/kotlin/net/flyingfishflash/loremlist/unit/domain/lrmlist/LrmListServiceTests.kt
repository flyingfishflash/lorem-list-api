...
class LrmListServiceTests : DescribeSpec({
  ...
  describe("create()") {
    it("list repository returns inserted list id") {
      every { mockLrmListRepository.insert(ofType(LrmListRequest::class)) } returns id1
  ...
  describe("deleteById()") {
    it("list not found") {
      ...
      val exception = shouldThrow<ApiException> { lrmListService.deleteById(id1, removeItemAssociations = false) }
    ...
    describe("associated items") {
      it("list is deleted (removeItemAssociations = true)") {
        every { mockLrmListRepository.findByIdOrNull(id1) } returns lrmList()
        every { mockLrmListRepository.findByIdOrNullIncludeItems(id1) } returns lrmListWithItems()
        every { mockAssociationService.deleteAllOfList(id1) } returns Pair(lrmList().name, 999)
        every { mockLrmListRepository.deleteById(id1) } returns 1
        lrmListService.deleteById(id1, removeItemAssociations = true)
        verify(exactly = 1) { mockLrmListRepository.findByIdOrNull(any()) }
      ...
      it("list is not deleted (removeItemAssociations = false)") {
        ...
        val exception = shouldThrow<ApiException> {
          lrmListService.deleteById(id1, removeItemAssociations = false)
        }
      ...
      it("list repository returns > 1 deleted records") {
        every { mockLrmListRepository.findByIdOrNull(id1) } returns lrmList()
        every { mockLrmListRepository.findByIdOrNullIncludeItems(id1) } returns lrmListWithItems()
        every { mockAssociationService.deleteAllOfList(id1) } returns Pair("Lorem Ipsum", 999)
        every { mockLrmListRepository.deleteById(id1) } returns 2
        val exception = shouldThrow<ApiException> { lrmListService.deleteById(id1, removeItemAssociations = true) }
    ...
    describe("no associated items") {
      it("list is deleted") {
        every { mockLrmListRepository.findByIdOrNull(id1) } returns lrmList()
        every { mockLrmListRepository.findByIdOrNullIncludeItems(id1) } returns lrmList()
        every { mockLrmListRepository.deleteById(id1) } returns 1
        lrmListService.deleteById(id1, removeItemAssociations = false)
        verify(exactly = 1) { mockLrmListRepository.findByIdOrNullIncludeItems(any()) }
        verify(exactly = 1) { mockLrmListRepository.deleteById(id1) }
      }
  ...
  describe("findById()") {
    it("list is found and returned") {
      every { mockLrmListRepository.findByIdOrNull(id1) } returns lrmList()
      val result = lrmListService.findById(id1)
    ...
    it("list is not found") {
      ...
      assertThrows<ListNotFoundException> {
        lrmListService.findById(id1)
      }
    }

    it("list repository throws exception") {
      every { mockLrmListRepository.findByIdOrNull(id1) } throws Exception("Lorem Ipsum")
      val exception = shouldThrow<ApiException> { lrmListService.findById(id1) }
      exception.cause.shouldBeInstanceOf<Exception>()
      exception.httpStatus.shouldBe(HttpStatus.INTERNAL_SERVER_ERROR)
      exception.message
        .shouldContainIgnoringCase("$id1")
  ...
  describe("findByIdIncludeItems()") {
    it("list is found and returned") {
      every { mockLrmListRepository.findByIdOrNullIncludeItems(id1) } returns lrmList()
      val result = lrmListService.findByIdIncludeItems(id1)
      result.shouldBe(lrmList())
      verify(exactly = 1) { mockLrmListRepository.findByIdOrNullIncludeItems(id1) }
    }

    it("list is not found") {
      every { mockLrmListRepository.findByIdOrNullIncludeItems(id1) } returns null
      assertThrows<ListNotFoundException> {
        lrmListService.findByIdIncludeItems(id1)
      }
    }

    it("list repository throws exception") {
      every { mockLrmListRepository.findByIdOrNullIncludeItems(id1) } throws Exception("Lorem Ipsum")
      val exception = shouldThrow<ApiException> { lrmListService.findByIdIncludeItems(id1) }
      exception.cause.shouldBeInstanceOf<Exception>()
      exception.httpStatus.shouldBe(HttpStatus.INTERNAL_SERVER_ERROR)
      exception.message
        .shouldContainIgnoringCase("$id1")
  ...
  describe("patch()") {
    it("list is not found") {
      every { mockLrmListRepository.findByIdOrNull(id1) } returns null
      shouldThrow<ListNotFoundException> { lrmListService.patch(id1, mapOf("name" to "lorum ipsum")) }
    }
    ...
    it("update name") {
      ...
      val patchedLrmList = lrmListService.patch(id1, mapOf("name" to expectedName)).first
    ...
    it("update name and description to current values") {
      ...
      every { mockLrmListRepository.findByIdOrNull(id1) } returns lrmList()
      every { mockLrmListRepository.update(ofType(LrmList::class)) } returns 1
      val patchResponse = lrmListService.patch(id1, mapOf("name" to expectedName, "description" to (expectedDescription ?: "")))
      ...
      verify(exactly = 1) { mockLrmListRepository.findByIdOrNull(id1) }
    ...
    it("update description") {
      ...
      every { mockLrmListRepository.findByIdOrNull(id1) } returns lrmList()
      every { mockLrmListRepository.update(ofType(LrmList::class)) } returns 1
      val patchedLrmList = lrmListService.patch(id1, mapOf("description" to expectedDescription)).first
    ...
    it("update an undefined list property") {
      ...
      shouldThrow<IllegalArgumentException> {
        lrmListService.patch(id1, mapOf("undefined property" to "irrelevant value"))
      }
    }

    it("update no properties") {
      every { mockLrmListRepository.findByIdOrNull(id1) } returns lrmList()
      val patchReturn = lrmListService.patch(id1, mapOf())
    ...
    it("list repository updates more than 1 record") {
      ...
      val exception = shouldThrow<ApiException> { lrmListService.patch(id1, mapOf("name" to expectedName)).first }
      exception.httpStatus.shouldBe(HttpStatus.INTERNAL_SERVER_ERROR)
