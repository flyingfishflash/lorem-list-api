...
class LrmListFunctionalTests(mockMvc: MockMvc) :
  DescribeSpec({

    data class ValidationTest(val postContent: String, val responseMessage: String, val expectedErrorCount: Int)

    fun createLrmItemOneRequest(): LrmItemCreateRequest = LrmItemCreateRequest("Lorem Item 1 Name", "Lorem Item 1 Description", 0)
    fun createLrmItemTwoRequest(): LrmItemCreateRequest = LrmItemCreateRequest("Lorem Item 2 Name", "Lorem Item 2 Description", 0)
    fun createLrmItemThreeRequest(): LrmItemCreateRequest = LrmItemCreateRequest("Lorem Item 3 Name", "Lorem Item 3 Description", 0)
    val itemCreateRequests = listOf(createLrmItemOneRequest(), createLrmItemTwoRequest(), createLrmItemThreeRequest())

    fun updateLrmItemOneRequest(): LrmItemCreateRequest =
      LrmItemCreateRequest("Updated Lorem Item 1 Name", "Updated Lorem Item 1 Description", 1001)
    fun updateLrmItemTwoRequest(): LrmItemCreateRequest =
      LrmItemCreateRequest("Updated Lorem Item 2 Name", "Updated Lorem Item 2 Description", 1002)
    fun updateLrmItemThreeRequest(): LrmItemCreateRequest =
      LrmItemCreateRequest("Updated Lorem Item 3 Name", "Updated Lorem Item 3 Description", 1003)
          ...
          itemCreateRequests.forEachIndexed { index, itemRequest ->
            it("item $index is found with all items") {
              ...
              mockMvc.get(instance) {
                ...
                jsonPath("$.disposition") { value(DispositionOfSuccess.SUCCESS.nameAsLowercase()) }
                jsonPath("$.method") { value(HttpMethod.GET.name().lowercase()) }
                jsonPath("$.message") { value("Retrieved all items owned by user.") }
                jsonPath("$.instance") { value(instance) }
...
//            jsonPath("$.content.detail") { value(expectedMessage) }
      ...
      describe("item: list association: create, read, update, delete") {
        it("item 0 is added to list 0") {
          val instance = "/items/${itemIds[0]}/lists"
          mockMvc.post(instance) {
            with(jwt())
            with(csrf())
        ...
        it("item 1 is added to lists 0 and 1") {
          ...
          val instance = "/items/${itemIds[1]}/lists"
          mockMvc.post(instance) {
            with(jwt())
            with(csrf())
        ...
        it("an item is not added to list 0 because the item couldn't be found") {
          val itemId = UUID.randomUUID()
          val instance = "/items/$itemId/lists"
          mockMvc.post(instance) {
            with(jwt())
            with(csrf())
            content = Json.encodeToString(listOf(listIds[0].toString()))
        ...
        it("item 0 is moved from list 0 to list 1") {
          val instance = "/items/${itemIds[0]}/lists/${listIds[0]}/${listIds[1]}"
          mockMvc.patch(instance) {
            with(jwt())
            with(csrf())
            contentType = MediaType.APPLICATION_JSON
        ...
        it("list 0 includes only item 1") {
          ...
          mockMvc.get(instance) {
            ...
            jsonPath("$.content.items.[0].lists") { isEmpty() }
          }
        }

        it("item 0 is not deleted from list 0 because the association couldn't be found") {
          val instance = "/items/${itemIds[0]}/lists/${listIds[0]}"
...
//          content = "\"${listIds[0]}\""
        ...
        it("item 0 is associated with only list 1") {
          ...
          mockMvc.get(instance) {
            ...
            jsonPath("$.disposition") { value(DispositionOfSuccess.SUCCESS.nameAsLowercase()) }
            jsonPath("$.method") { value(HttpMethod.GET.name().lowercase()) }
            jsonPath("$.message") { value("Retrieved item '${updateLrmItemOneRequest().name}'") }
            jsonPath("$.instance") { value(instance) }
            jsonPath("$.size") { value(1) }
        ...
        it("item 0 has one list association") {
          val instance = "/items/${itemIds[0]}/lists/count"
          mockMvc.get(instance) {
            with(jwt())
            contentType = MediaType.APPLICATION_JSON
          }.andExpect {
        ...
        it("item 1 has two list associations") {
          val instance = "/items/${itemIds[1]}/lists/count"
          mockMvc.get(instance) {
            with(jwt())
            contentType = MediaType.APPLICATION_JSON
          }.andExpect {
        ...
        it("list 0 has one item association") {
          val instance = "/lists/${listIds[0]}/items/count"
          mockMvc.get(instance) {
            with(jwt())
            contentType = MediaType.APPLICATION_JSON
          }.andExpect {
        ...
        it("list 1 has two item association") {
          val instance = "/lists/${listIds[1]}/items/count"
          mockMvc.get(instance) {
            with(jwt())
            contentType = MediaType.APPLICATION_JSON
          }.andExpect {
        ...
        it("remove item 0 from all lists") {
          ...
          mockMvc.delete(instance) {
            ...
            jsonPath("$.disposition") { value(DispositionOfSuccess.SUCCESS.nameAsLowercase()) }
            jsonPath("$.method") { value(HttpMethod.DELETE.name().lowercase()) }
            jsonPath("$.message") { value("Removed '${updateLrmItemOneRequest().name}' from 1 list.") }
            jsonPath("$.instance") { value(instance) }
            jsonPath("$.size") { value(1) }
            jsonPath("$.content.length()") { value(1) }
            jsonPath("$.content.value") { value(1) }
        ...
        it("remove item 1 from list 1") {
          ...
          val instance = "/items/${itemIds[1]}/lists/${listIds[1]}"
          mockMvc.delete(instance) {
            with(jwt())
            with(csrf())
            contentType = MediaType.APPLICATION_JSON
      ...
      describe("list: item association: create, read, delete") {
        ...
        it("item 0 is added to list 1") {
          val instance = "/lists/${listIds[1]}/items"
          mockMvc.post(instance) {
            with(jwt())
            with(csrf())
        ...
        it("item 2 is deleted from list 1") {
          val instance = "/lists/${listIds[1]}/items/${itemIds[2]}"
          mockMvc.delete(instance) {
            with(jwt())
            with(csrf())
            contentType = MediaType.APPLICATION_JSON
        ...
        it("item 0 is not added to a list because the list couldn't be found") {
          val listId = UUID.randomUUID()
          val instance = "/lists/$listId/items"
          mockMvc.post(instance) {
            with(jwt())
            with(csrf())
            content = Json.encodeToString(listOf(itemIds[0].toString()))
      ...
      describe("item: delete") {
        it("item 2 has zero list associations") {
          val instance = "/items/${itemIds[2]}/lists/count"
          mockMvc.get(instance) {
            with(jwt())
            contentType = MediaType.APPLICATION_JSON
          }.andExpect {
        ...
        it("delete item 1 (removeListAssociations = false)") {
          ...
          mockMvc.delete(instance) {
            ...
            jsonPath("$.message") { isNotEmpty() }
            jsonPath("$.size") { value(1) }
            jsonPath("$.content.detail") { isNotEmpty() }
            jsonPath("$.content.cause") { isNotEmpty() }
            jsonPath("$.content.cause.name") { value(DomainException::class.java.simpleName) }
            jsonPath("$.content.cause.message") { isNotEmpty() }
          }
        }
      ...
      describe("list: delete") {
        ...
        it("item 0 is added to list 0") {
          val instance = "/lists/${listIds[0]}/items"
          mockMvc.post(instance) {
            with(jwt())
            with(csrf())
            content = Json.encodeToString(listOf(itemIds[0].toString()))
