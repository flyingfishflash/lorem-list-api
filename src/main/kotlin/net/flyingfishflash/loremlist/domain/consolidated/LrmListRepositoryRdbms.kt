...
class LrmListRepositoryRdbms : LrmListRepository {
  ...
  override fun findByOwner(owner: String): List<LrmList> = repositoryTable.selectAll()
    .where { repositoryTable.createdBy eq owner }
    .map { it.toLrmList() }.toList()

  override fun findByOwnerIncludeItems(owner: String): List<LrmList> {
  ...
  override fun findByOwnerAndIdOrNull(id: UUID, owner: String): LrmList? = repositoryTable
    .selectAll()
  ...
  override fun findByOwnerAndIdOrNullIncludeItems(id: UUID, owner: String): LrmList? {
  ...
  override fun findByOwnerAndHavingNoItemAssociations(owner: String): List<LrmList> {
  ...
  override fun findByPublic(): List<LrmList> = repositoryTable.selectAll().where {
    repositoryTable.public.eq(true)
  }.map { it.toLrmList() }.toList()

  override fun findByPublicIncludeItems(): List<LrmList> {
    val resultRows = (repositoryTable leftJoin LrmListsItemsTable leftJoin LrmListItemTable)
      .selectAll()
  ...
  override fun findIdsByOwnerAndIds(listIdCollection: List<UUID>, owner: String): List<UUID> {
  ...
  override fun insert(lrmListCreateRequest: LrmListCreateRequest, subject: String): UUID {
    val now = now()
    val id =
      repositoryTable
        .insertAndGetId {
  ...
  override fun update(lrmList: LrmList): Int {
    val updatedCount =
