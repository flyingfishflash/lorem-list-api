...
class CoreExceptionHandler(private val environment: Environment) : ResponseEntityExceptionHandler() {
  companion object {
  ...
  fun handleAbstractCoreException(request: HttpServletRequest, exception: CoreException): ResponseEntity<ResponseProblem> {
  ...
  fun handleConstraintViolationException(request: HttpServletRequest, exception: ConstraintViolationException): ResponseEntity<ResponseProblem> {
    ...
    val apiProblemDetail = ApiProblemDetail(
      type = CoreException.DEFAULT_PROBLEM_TYPE.toString(),
